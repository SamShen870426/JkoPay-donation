...
type CharityProductParams = { Params: { id: string } };
...
export function createDonationController(service: DonationService): DonationController {
  return {
    ...
    async getCharityProduct(request, reply) {
      let body;
      try {
        body = await service.getCharityProductDetail(request.params.id);
      } catch (e) {
        if (e instanceof AppError) throw e;
        request.log.error(e);
        throw new AppError('CHARITY_PRODUCT_DETAIL_FAILED', 'Failed to load charity product', 500);
      }

      const out = charityProductDetailSchema.safeParse(body);
      if (!out.success) {
        request.log.error(out.error, 'Response contract mismatch');
        throw new AppError(
          'RESPONSE_CONTRACT_MISMATCH',
